...
let content = fs.readFileSync('src/lib/escposCargoReceiptPrinting.ts', 'utf8');
...
content = content.replace(
  /export async function compileCargoReceiptStream.*?return concatChunks\(chunks\);\n\}/s,
  replacement
);

fs.writeFileSync('src/lib/escposCargoReceiptPrinting.ts', content);
console.log('Receipt patched');
