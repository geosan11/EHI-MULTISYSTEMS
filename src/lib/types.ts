...
export type TabView = 'Tower' | 'Cargo' | `Baggage:${string}` | `More:${string}` | 'Marketing' | 'Packages' | 'Scan' | 'Incoming' | 'IncomingToHub' | 'OutboundArrivals' | 'More' | 'MyTrips' | 'IT Debug' | 'Credit & Debit' | 'AirlineLogos' | 'DataImport' | 'AirlineLedger' | 'WeightManifest' | 'AirlinePerformance' | 'GAT' | 'FlightRadar';
