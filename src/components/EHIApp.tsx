  ...
  const fetchInitial = useCallback(async () => {
    ...
    try {
        ...
        const [shifts, cargoRes, baggageRes, mktRes, packageRes, expRes, profilesRes, debtCollectionRes] = await Promise.all([
          ...
          // landed (not the original sale's created_at), so a debt from
          // outside this date window that gets cleared today still shows
          // up under today. See supabase/migrations/
          // 20260941_debt_collection_events.sql for why these are never
          // real rows in cargo_entries/manifests/marketing_entries/
          // package_entries.
          supabase.rpc('debt_collection_events', {
            p_start: startISO,
            p_end: endISO,
            p_hub_id: (!isAdmin && user.hub_id && !stateWideView) ? user.hub_id : null,
          }),
        ]);
        ...
        const cargoRows = [...(cargoRes.data || []), ...dcByType.cargo];
        const baggageRows = [...(baggageRes.data || []), ...dcByType.baggage];
        ...
        if (cargoRows.length > 0) {
          cargoRows.forEach(r => {
            ...
            allTx.push({
              ...
              linked_as_office_work: r.linked_as_office_work ?? undefined,
        ...
        if (baggageRows.length > 0) {
          baggageRows.forEach(r => {
            ...
            allTx.push({
              ...
              // is_debt_clearance/related_tx_id: same COLLECTION-badge gap
              // fixed for cargo above, now closed for baggage too (see
              ...
              remarks: r.remark || undefined,
              // client_type/corporate_client_id/linked_as_office_work:
              // selected since 20260926_office_work_all_departments.sql but
              // never mapped onto the Transaction object for this
              // department -- the main Ledger's "Office Work" filter
              // (clientType/linked_as_office_work) silently only ever
              // matched Cargo entries as a result.
              clientType: r.client_type || undefined,
              corporate_client_id: r.corporate_client_id || undefined,
              linked_as_office_work: r.linked_as_office_work ?? undefined,
              raw: r,
        ...
        if (mktRows.length > 0) {
          mktRows.forEach((r: any) => {
            ...
            allTx.push({
              ...
              remarks: r.remark || undefined,
              // client_type/corporate_client_id/linked_as_office_work:
              // same baggage-department mapping gap fixed above.
              clientType: r.client_type || undefined,
              corporate_client_id: r.corporate_client_id || undefined,
              linked_as_office_work: r.linked_as_office_work ?? undefined,
              raw: r,
